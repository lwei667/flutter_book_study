...
class PopoverPositionRenderObject extends RenderShiftedBox {
  ...
  double _horizontalOffset(Size size) {
    ...
    if (attachRect.left + (attachRect.right - attachRect.left) / 2 > size.width / 2 &&
        Utils().screenWidth - attachRect.right - (attachRect.left - attachRect.right) / 2 > size.width / 2) {
      offset = attachRect.left + attachRect.width / 2 - size.width / 2;
    } else if (attachRect.left < size.width / 2) {
      offset = horizontalOffset;
    } else {
      offset = Utils().screenWidth - horizontalOffset - size.width;
    }
    return offset;
  }
